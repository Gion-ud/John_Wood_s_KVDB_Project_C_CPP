...
HTObject *HASH_INDEX_LIB_NewHashTableObject(int ht_cap) {
...
void HASH_INDEX_LIB_DestroyHashTableObject(HTObject *ht_obj) {
//#define ht_obj (*ht_obj_p)
    print_dbg_msg("HASH_INDEX_LIB_DestroyHashTableObject\n");
    if (!ht_obj) return;
...
HTObject* HASH_INDEX_LIB_ResizeHashTable(HTObject **ht_obj_p, int new_cap) {
#define ht_obj (*ht_obj_p)
    if (!ht_obj) return NULL;
    HTObject *ht_obj_tmp = HASH_INDEX_LIB_NewHashTableObject(new_cap);
    if (!ht_obj_tmp) return NULL;
    for (size_t i = 0; i < ht_obj->ht_cap; ++i) {
        if (!ht_obj->ht[i].bucket) continue;
        for (size_t j = 0; j < ht_obj->ht[i].bucket_size; j++)
            HASH_INDEX_LIB_InsertHashTableEntry(ht_obj_tmp, &ht_obj->ht[i].bucket[j]);
    }
    HASH_INDEX_LIB_DestroyHashTableObject(ht_obj); ht_obj = ht_obj_tmp;
    return ht_obj;
#undef ht_obj
}


int HASH_INDEX_LIB_InsertHashIndexEntry(HTObject *ht_obj, const HashTableEntry *ht_entry) {
//#define ht_obj (*ht_obj_p)
    print_dbg_msg("HASH_INDEX_LIB_InsertHashTableEntry\n");
    if (!ht_obj) return -1;
...
int HASH_INDEX_LIB_GetEntryID(HTObject *ht_obj, void *key, size_t key_len) {
    if (!ht_obj) return NULL;
    ...
    for (size_t i = 0; i < ht_obj->ht[h_idx].bucket_size; ++i) {
        if (ht_obj->ht[h_idx].bucket[i].key_data &&
            ...
            return ht_obj->ht[h_idx].bucket[i].entry_id;
        }
    }
    return NULL;
}

void HASH_INDEX_LIB_DestroyTLVDataObject(TLVDataObject *buf) {
    if (!buf) return;
    if (buf->data) {
...
int HASH_INDEX_LIB_DeleteHashIndexEntry(HTObject *ht_obj, void *key, size_t key_len) {
    if (!ht_obj) return -1;
    ...
    for (size_t i = 0; i < ht_obj->ht[h_idx].bucket_size; ++i) {
        if (
            ht_obj->ht[h_idx].bucket[i].key_data &&
            memcmp(ht_obj->ht[h_idx].bucket[i].key_data, key, key_len) == 0 &&
            ht_obj->ht[h_idx].bucket[i].key_len == key_len
        ) {
            if (ht_obj->ht[h_idx].bucket[i].key_data) {
