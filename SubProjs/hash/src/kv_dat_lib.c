...
const ubyte_t Magic[MAGIC_LEN] = { 'K', 'V', ' ', 'D', 'A', 'T', '\r', '\n' };
...
#define idx pObj->kv_cnt
    ...
    pObj->kv_arr[idx].key_data = (ubyte_t*)malloc(key_len);
    if (!pObj->kv_arr[idx].key_data) { print_err_msg("(ubyte_t*)malloc(key_len) failed\n"); return -3; }

    pObj->kv_arr[idx].val_data = (ubyte_t*)malloc(val_len);
    if (!pObj->kv_arr[idx].val_data) { print_err_msg("(ubyte_t*)malloc(val_len) failed\n"); free(pObj->kv_arr[idx].key_data); return -3; }
...
//\
    ...
    for (ulong_t i = 0; i < kv_dat_hdr.entry_cnt; i++) {
        ...
        ubyte_t *key_data = (ubyte_t*)malloc(key_len);
        if (!key_data) { perror("malloc"); goto err_cleanup; }
        fread(key_data, sizeof(ubyte_t), key_len, fp);

        fread(&val_len, sizeof(val_len), 1, fp);
        ubyte_t *val_data = (ubyte_t*)malloc(val_len);
        if (!val_data) { perror("malloc"); free(key_data); goto err_cleanup; }
        fread(val_data, sizeof(ubyte_t), val_len, fp);
