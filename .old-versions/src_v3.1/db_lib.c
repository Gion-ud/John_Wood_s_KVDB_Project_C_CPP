...
static const byte_t pad_arr[8] = {0};
    ...
    fputs("Header.Magic=", fp );
{
    ...
    fprintf(
        ...
        "Header.DataEntryHeaderSize=%llu\n\n",
        Header.Version,
        Header.ByteOrder,
        ...
        (unsigned long long)Header.DataEntryHeaderSize
    );
    if (fp == stdout) { printf(ESC RESET_COLOUR); }
    if (fp == stderr) { print_dbg_msg(ESC RESET_COLOUR); }
}
static void PrintIndexEntry(FILE* fp, DBObject db, uint32_t EntryID) {
    if (fp == stderr) { PRINT_DBG_MSG(ESC COLOUR_CYAN); }
    fprintf(fp, "# IndexTable[%u]\n", EntryID);
    fprintf(
        fp,
        "IndexTable[%u].KeyHash=0x%.16llx\n"
        "IndexTable[%u].Flags=0x%08X\n"
        "IndexTable[%u].Size=%llu\n"
        "IndexTable[%u].Offset=%llu\n\n",
        EntryID, (unsigned long long)db.IndexTable[EntryID].KeyHash,
...
static inline void PrintIndexTable(FILE* fp, DBObject db) {
    fputs("# IndexTable\n", fp);
...
//  # This function creates a new db object\
    ...
    if (MaxEntryCount > INT32_MAX) { print_err_msg("MaxEntryCount:i32 overflow\n"); return NULL; };
...
#define db (*dbp)
    ...
    //memcpy(db.Header._pad0, (byte_t*)pad_arr, sizeof(db.Header._pad0));
    //memcpy(db.Header._pad1, (byte_t*)pad_arr, sizeof(db.Header._pad1));

    PrintHeader(stderr, db.Header);

    /*
    db.HashTable = (hash_t*)calloc(db.Header.MaxEntryCount, sizeof(hash_t));
    if (!db.HashTable) {
        PRINT_DBG_MSG("hash_t*)calloc(db.Header.MaxEntryCount, sizeof(hash_t)) failed\n");
        free(dbp); return NULL;
    }
    */
    ...
    if (!db.IndexTable) {
        print_err_msg("(DBIndexEntry*)calloc(MaxEntryCount * INDEX_ENTRY_SIZE) failed\n");
        CloseDB(&db); return NULL;
    }

    db.key_arr = (Key*)calloc(MaxEntryCount, sizeof(Key));
    if (!db.key_arr) {
        print_err_msg("(Key*)calloc(MaxEntryCount, sizeof(Key)) failed\n");
        CloseDB(&db); return NULL;
    }

    db.EntryID_arr_htidx = (int*)calloc(db.Header.MaxEntryCount, sizeof(int));
    if (!db.EntryID_arr_htidx) {
        PRINT_DBG_MSG("(int*)calloc(db.Header.MaxEntryCount, sizeof(int)) failed\n");
        CloseDB(&db); return NULL;
    }
    ...
    int ret = InitHashTableObject(db.htObj, db.Header.MaxEntryCount);
    if (ret < 0) { CloseDB(&db); return NULL; }
...
#define db (*dbp)
    db.filepath = (char*)malloc(FILE_PATH_SIZE);
    if (!db.filepath) { perror("malloc(FILE_PATH_SIZE)"); CloseDB(&db); return NULL; }
    memcpy((char*)db.filepath, filepath, FILE_PATH_SIZE);

    db.fp = fopen(db.filepath, "rb+");
    if (!db.fp) { perror("fopen(db.filepath, \"rb+\")"); CloseDB(&db); return NULL; }
    rewind(db.fp);
    ...
    /*
    db.HashTable = (hash_t*)calloc(db.Header.MaxEntryCount, sizeof(hash_t));
    if (!db.HashTable) { PRINT_DBG_MSG("hash_t*)calloc(db.Header.MaxEntryCount, sizeof(hash_t)) failed\n"); exit(EXIT_FAILURE); }
    */

    db.IndexTable = (DBIndexEntry*)calloc(db.Header.MaxEntryCount, INDEX_ENTRY_SIZE);
    if (!db.IndexTable) { perror("(DBIndexEntry*)calloc(MaxEntryCount * INDEX_ENTRY_SIZE)"); CloseDB(&db); return NULL; }
    fseek(db.fp, db.Header.HeaderSize, SEEK_SET);
    for (int i = 0; i < db.Header.EntryCount; i++) {
        fread(&db.IndexTable[i], INDEX_ENTRY_SIZE, 1, db.fp);
        //db.HashTable[i] = db.IndexTable[i].KeyHash;
    }
    ...
    db.EntryID_arr_htidx = (int*)calloc(db.Header.MaxEntryCount, sizeof(int));
    if (!db.EntryID_arr_htidx) {
        print_err_msg("(int*)calloc(db.Header.MaxEntryCount, sizeof(int)) failed\n");
        CloseDB(&db); return NULL;
    }
    for (uint32_t i = 0; i < db.Header.MaxEntryCount; i++) {
        db.EntryID_arr_htidx[i] = -1;
    ...
    if (ret < 0) { CloseDB(&db); return NULL; }
    for (uint32_t i = 0; i < db.Header.EntryCount; i++) {
        ret = insert_entry(db.htObj, (byte_t*)db.key_arr[i].data, (keysize_t)db.key_arr[i].size);
...
void CloseDB(DBObject* dbp) {
    ...
    if (DB.key_arr) free(DB.key_arr);
    if (DB.htObj) {
        DestroyHashTableObject(DB.htObj);
        free(DB.htObj);
    }
    if (DB.HashTable) free(DB.HashTable);
    if (DB.EntryID_arr_htidx)free(DB.EntryID_arr_htidx);
...
void ReadDBEntryKey(DBObject* dbp, Key key) {
    ...
    ReadDBEntry(&DB, (unsigned int)id); // read db entry by idx
}
