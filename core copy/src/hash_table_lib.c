...
hash_t hash(const ubyte_t* key, keysize_t size) {
...
#define table_size  pObj->hash_table_size
    //print_dbg_msg("[DEBUG] hidx_t insert_entry(HashTableObject* pObj, const ubyte_t* key, keysize_t key_size);\n");
    ...
    while (LOOP_RUNNING) {
        if (HashTable[hash_idx].flag == ENTRY_EMPTY || HashTable[hash_idx].flag == ENTRY_DELETED) {
            ...
            HashTable[hash_idx].key = (ubyte_t*)malloc(key_size * sizeof(ubyte_t));
            if (!HashTable[hash_idx].key) {
                print_err_msg("(ubyte_t)malloc(key_size * sizeof(ubyte_t)) failed\n");
